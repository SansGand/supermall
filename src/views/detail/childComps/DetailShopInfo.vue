      ...
      <div class="shop-middle-item shop-middle-right">
        <table>
          <tr v-for="(item,index) in shops.score" :key="index">
            ...
            <td class="better" :class="{'better-better':item.isBetter}">{{item.isBetter ? '高':'低'}}</td>
          </tr>
        </table>
      </div>
    </div>
...
export default {
  ...
  filters: {
    // 大于10000的显示为1万
    sellCountFilter(vaule) {
      if (vaule < 10000) return vaule
      return (vaule/10000).toFixed(1) + '万'
    }
  }
}
</script>
...
<style  scoped>
  ...
  .shop-middle-right .better {
   ...
   .shop-middle-right .better-better {
